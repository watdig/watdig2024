...
class BackUWB(Node):
    def __init__(self):
        super().__init__('back_uwb_node')
        self.front_uwb_publisher = self.create_publisher(Float32MultiArray, 'back_uwb_topic', 10)
        self.serial_port = serial.Serial('EDIT THE RIGHT PORT', 115200, timeout=0.1)
...
def main(args=None):
    ...
    back_uwb_node = BackUWB()
    executor = MultiThreadedExecutor()
    rclpy.spin(back_uwb_node, executor=executor)

    back_uwb_node.serial_port.close()
    rclpy.shutdown()
